...
async def perform_task(request_data: TextInput):
    ...
    system_content = "The following object represents cumulative bias that a reader has been exposed to over a series of articles. Come up with an ANALYSIS of the social biases that the user is exposing themselves to over time. Pay particular attention to the subtle ways in which biases show up and reinforce a particular worldview. Return your analysis as a short, readable analysis. Keep in mind this analysis is over a SERIES of articles. You are generating a report on the RANGE of different biases the reader has been exposed to. Your response should be returned as styled HTML. Use different header sizes, unordered lists, list items, etc."
    model='gpt-4o-mini'
